...
for train_file, test_file, od_model in product(synthetic_train_files, syncan_test_files, od_models):
    ...
    with open(os.path.join("syncan_od_jobs", job_name+".job"), 'w') as f:
        ...
        f.write(
            f"python3 syncan-outlier-detection-sklearn.py --train_csv_path {train_file} --test_csv_path {test_file} --results_json_file {results_json_file} --model_type {od_model} --sample_size 0.01\n")
        f.close()
